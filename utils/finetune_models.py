    ...
    def finetune_on_pseudos_NNCLR(self, input_dim, x, y, x_label_points, y_pred_labelled_points,
        ...
        finetuning_model = tf.keras.Sequential(
            [
                ...
                ConditionalAugmentation(augmenter(**classification_augmenter)),
                self.pseudo,
        ...
        y_pred = finetuning_model.predict(x).argmax(1)
...
def finetune_model(model, finetune_config, dataset, dataset_test):
    """
    ...
    for stept, batcht in enumerate(dataset_test):
        if step == 0:
            ...
            x_test = np.concatenate((x_test, batcht[0]), axis=0)
            y_test = np.concatenate((y_test, batcht[1]), axis=0)
    ...
    #test
    y_pred_test = finetuning_model.predict(x_test).argmax(1)
    final_acc = acc(y_test, y_pred_test)
    print("Test_acc:", final_acc)
    wandb.log({"Test_acc": final_acc})
